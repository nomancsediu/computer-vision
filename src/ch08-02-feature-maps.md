...
# একটি specific layer এর feature map extract করার মডেল
...
print(f"Feature map model তৈরি হয়েছে: {layer_name}")
...
# ইমেজ লোড
img_path = 'sample_image.jpg'  # তোমার ইমেজের path
...
# Output: (1, 224, 224, 64) — 64টি feature map, প্রতিটি 224×224

# প্রথম 16টি feature map plot
n_features = 16
...
```python
# যেসব layer এর feature map দেখবো
layer_names = [
    'block1_conv2',   # Block 1 শেষ (shallow)
    'block2_conv2',   # Block 2 শেষ
    'block3_conv3',   # Block 3 শেষ (middle)
    'block4_conv3',   # Block 4 শেষ
    'block5_conv3'    # Block 5 শেষ (deep)
]

# মাল্টি-output model তৈরি
...
# একসাথে সব layer এর feature map
feature_maps_list = multi_output_model.predict(img_preprocessed)

# প্রতিটি layer থেকে 8টি feature map plot
...
plt.suptitle('VGG16 Feature Maps — Shallow থেকে Deep', fontsize=14, y=1.02)
plt.tight_layout()

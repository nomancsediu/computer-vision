...
Window 1 (top-left):  max(1,3,5,6) = 6
Window 2 (top-right):  max(2,1,0,4) = 4
Window 3 (bottom-left):  max(3,2,1,0) = 3
Window 4 (bottom-right):  max(8,7,3,4) = 8
...
```
Input: 28×28×32  →  2×2 MaxPool  →  Output: 14×14×32
Input: 14×14×64  →  2×2 MaxPool  →  Output: 7×7×64
Input: 224×224×64  →  2×2 MaxPool  →  Output: 112×112×64
```
...
# If strides not specified, default = pool_size
# The following two are equivalent:
pool = MaxPooling2D(pool_size=(2, 2), strides=2)
pool = MaxPooling2D(pool_size=(2, 2))  # strides automatically (2,2)

# Specify padding (default='valid')
...
gap = GlobalAveragePooling2D()  # Collapses entire spatial dimension into a 1D vector
```
...
model_pooling = Sequential([
    Conv2D(64, (3, 3), activation='relu', padding='same'),
    MaxPooling2D((2, 2), strides=2),  # No learnable parameters
])

# Downsampling with strided convolution
model_strided = Sequential([
    Conv2D(64, (3, 3), activation='relu', padding='same', strides=2),  # Has learnable parameters
])
```

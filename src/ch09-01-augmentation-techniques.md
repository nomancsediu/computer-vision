...
# Augmentation-enabled generator for training data
train_datagen = ImageDataGenerator(
    rescale=1./255,                  # Pixel value 0-255 → 0-1 normalize
    rotation_range=20,               # Random rotation from -20° to +20°
    shear_range=0.2,                 # 0.2 radian shear intensity
    zoom_range=0.2,                  # Random zoom from 80% to 120%
    horizontal_flip=True,            # Horizontal flip (left-right)
    vertical_flip=False,             # Vertical flip off (decide based on task)
    width_shift_range=0.2,           # Horizontal shift up to 20% of width
    height_shift_range=0.2,          # Vertical shift up to 20% of height
    fill_mode='nearest',             # Method to fill empty space after shift
    brightness_range=[0.8, 1.2]      # Brightness variation from 80% to 120%
)
...
# Create augmentation generator (only augmentation, no rescale  for visualization)
...
# Generate and visualize 9 augmented images

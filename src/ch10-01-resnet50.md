...
# weights='imagenet' → ImageNet এ trained weights load হবে
model = ResNet50(weights='imagenet')

# Model summary দেখা (optional)
model.summary()
```
...
```python
# --- 2. Image Load ও Preprocess ---
def predict_with_resnet(img_path):
    """
    ResNet50 দিয়ে image classification করে।

    Parameters:
        img_path: ইমেজের file path

    Returns:
        top predictions with labels ও confidence
    """
    ...
    # preprocess_input() pixel values কে ImageNet training format এ convert করে
    # (0-255 → centered around 0, BGR channel order ইত্যাদি)
    ...
    # decode_predictions() raw prediction vector কে human-readable label এ convert
    # top=5 means top 5 most likely classes return হবে
...
def predict_and_visualize(img_path):
    """Image ও prediction result একসাথে visualize করে।"""
    # Image load
    img = image.load_img(img_path, target_size=(224, 224))
    img_array = image.img_to_array(img)
    img_array = np.expand_dims(img_array, axis=0)

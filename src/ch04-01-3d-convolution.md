...
Multiple Filters → Output Volume:
  ...
  Input:    H × W × C_in
  Filters:  C_out টি kernel, প্রতিটি k × k × C_in
  Output:   (H-k+1) × (W-k+1) × C_out

  উদাহরণ:
  Input:    32 × 32 × 3    (RGB ইমেজ)
  Filters:  16 টি 3×3×3 kernel
  Output:   30 × 30 × 16   (16টি feature map stacked)
```
...
Layer 2 (Conv: 32 filters, 3×3):
  Each filter: 3 × 3 × 16     ← খেয়াল করো! এখন depth=16
...
def conv3d(image, kernels):
    ...
    # Kernel depth ও input depth সমান কিনা চেক
    ...
    # প্রতিটি kernel এর জন্য আলাদা convolution
    for f in range(num_filters):
        ...
        for i in range(out_h):
            for j in range(out_w):
                # 3D patch (সব চ্যানেল একসাথে)
                patch = image[i:i+k_h, j:j+k_w, :]
                # সব মান গুণ ও যোগ → single number
...
# 4টি আলাদা 3D kernel তৈরি (C_out=4, k=3, C_in=3)
np.random.seed(42)
num_filters = 4
kernels = np.random.randn(num_filters, 3, 3, 3) * 0.1  # ছোট random মান
...
# প্রতিটি feature map visualize করা

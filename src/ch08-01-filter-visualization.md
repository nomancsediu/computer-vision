...
# ImageNet pretrained VGG16 লোড (শুধু convolutional part)
...
```python
# একটি specific layer এর filter extract
...
def normalize_filters(filters):
    """Filter value গুলো 0-1 range এ normalize করে।"""
    f_min = filters.min()
    f_max = filters.max()
...
# প্রথম conv layer এর filter extract
layer = model.layers[1]  # block1_conv1
filters, biases = layer.get_weights()
...
# প্রথম 16টি filter plot
n_filters = 16
fig, axes = plt.subplots(2, 8, figsize=(16, 4))

for i in range(n_filters):
    ax = axes[i // 8, i % 8]
    # প্রতিটি filter এর shape: (3, 3, 3) — RGB image
    f = filters_norm[:, :, :, i]
    ax.imshow(f)
    ax.set_title(f'Filter {i}', fontsize=9)
    ax.axis('off')

plt.suptitle('VGG16 Block1_Conv1 — প্রথম 16টি ফিল্টার', fontsize=14, y=1.02)
plt.tight_layout()
plt.savefig('vgg16_block1_filters.png', dpi=150, bbox_inches='tight')
plt.show()

print(f"মোট ফিল্টার সংখ্যা: {filters.shape[3]}")
```
...
```python
# প্রতিটি block এর প্রথম conv layer
block_layers = {
...
for row, (block_name, layer_name) in enumerate(block_layers.items()):
    ...
    for col in range(8):
        ...
        # Deep layer এর filter এ input channels > 3
        # তাই শুধু প্রথম 3 channel দেখাবো (pseudo-RGB)
        if f.shape[2] > 3:
            f = f[:, :, :3]
...
plt.suptitle('VGG16 — প্রতিটি Block এর প্রথম 8টি ফিল্টার', fontsize=14)
plt.tight_layout()

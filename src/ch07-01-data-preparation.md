...
# Kaggle API Setup
# Put kaggle.json file in ~/.kaggle/
os.environ['KAGGLE_CONFIG_DIR'] = os.path.expanduser('~/.kaggle')

# Install Kaggle (if not installed)
# !pip install kaggle

# Download apples-or-tomatoes dataset
...
│   │   ├── apple2.jpg
│   │   └── ...
│   └── tomato/         ← Tomato training images
│       ├── tomato1.jpg
│       ├── tomato2.jpg
│       └── ...
├── val/
│   ├── apple/          ← Apple validation images
│   └── tomato/         ← Tomato validation images
└── test/
...
# Split images of each class
for cls in ['apple', 'tomato']:
    src_folder = os.path.join(base_dir, cls)  # Main class folder
...
train_datagen = ImageDataGenerator(
    ...
    rotation_range=20,            # -20° to +20° random rotation
    ...
    validation_split=0.2          # 20% validation split (if no separate val folder)
)

# For validation data: only rescale, no augmentation!
...
train_generator = train_datagen.flow_from_directory(
    ...
    shuffle=True                            # Shuffle is essential for training
)
...
test_generator = test_datagen.flow_from_directory(
    ...
    batch_size=32,
    class_mode='binary',
    shuffle=False                           # Never shuffle during test!
)

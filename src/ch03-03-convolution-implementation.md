...
def simple_conv(image, kernel):
    ...
    output : 2D numpy array
        Convolved output image (H-k+1 × W-k+1)
    """

    # Step 1: Get image and kernel dimensions
    img_h, img_w = image.shape    # Image height and width
    k_h, k_w = kernel.shape       # Kernel height and width

    # Step 2: Calculate output image size
    # Formula: (H - k + 1) × (W - k + 1)  (for stride=1)
    out_h = img_h - k_h + 1
    out_w = img_w - k_w + 1

    # Step 3: Create output array (initialize with all 0s)
    output = np.zeros((out_h, out_w), dtype=np.float64)

    # Step 4: Rotate kernel by 180°
    # Mathematical convolution requires kernel flip
    # (cross-correlation doesn't need flip, but
    #  deep learning frameworks usually use cross-correlation)
    kernel_flipped = np.flipud(np.fliplr(kernel))

    # Step 5: Sliding window - slide kernel across the entire image
    for i in range(out_h):          # For each row
        for j in range(out_w):      # For each column

            # Extract current window (patch) from image
            # patch is the area kernel is covering
            patch = image[i:i+k_h, j:j+k_w]

            # Element-wise multiplication and sum
            # This is the core computation of convolution
...
# Define kernels for various filters
filters = {
...
for name, result in results.items():
    ...
    # Show absolute value of Sobel filter
    if "Sobel" in name:
...
def simple_conv_with_stride(image, kernel, stride=1):
    """
    2D convolution implementation with stride
    """
    ...
    for i in range(out_h):
        for j in range(out_w):
            # Calculate position according to stride
            r = i * stride
            c = j * stride
...
# See the effect of stride
img_small = img_gray[:100, :100]  # small crop

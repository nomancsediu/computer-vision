...
```
Input:  64 channels (output of first layer)
...
```
Parameters = 1,792 (calculated above)
```
...
def convnet_layer(input_volume, kernels, biases):
    """
    Complete ConvNet layer: Convolution + Bias + ReLU
...
def count_parameters(c_in, c_out, kernel_size):
    """
    Calculate parameter count for ConvNet layer
    """
...
# Example: 3 input channels, 64 filters, 3×3 kernel

...
# Load VGG16 base model without classification head
base_model = VGG16(
    weights='imagenet',          # ImageNet pretrained weights
    include_top=False,           # Exclude fully connected layers (top)
    input_shape=(224, 224, 3)    # Our input size
)
...
# Verify which layers are trainable and which are not
...
train_datagen = ImageDataGenerator(
    rescale=1./255,
    rotation_range=15,            # High rotation not logical for X-ray
    ...
    fill_mode='nearest',
    horizontal_flip=False,        # ❌ Cannot flip X-ray!
    vertical_flip=False           # ❌ No vertical flip either!
)
...
def predict_fracture(img_path, model):
    """
    Predicts bone fracture type from X-ray image.
    """
...
# First freeze all layers (protect previous training weights)
for layer in base_model.layers:
    layer.trainable = False

# Unfreeze layers of last block (block5)
for layer in base_model.layers:
...
for layer in base_model.layers:
    ...
    # Recompile with LOWER learning rate — extremely important!
model.compile(
    optimizer=Adam(learning_rate=1e-5),    # 0.001 → 0.00001 (100x less!)
